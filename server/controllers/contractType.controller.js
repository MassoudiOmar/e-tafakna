  ...
  const fetchData = async () => {
    ...
    try {
      ...
      await axios
        ...
        .then((result) => {
          //alert(lang)
          if (lang == "Francais") setContractName(result.data[0].title_FR);
          if (lang == "Arabe") setContractName(result.data[0].title_AR);
          else setContractName(result.data[0].title_EN);
        });
    } catch (error) {
    ...
    if (currentQuestionIndex == questions.length - 1) {
      ...
      try {
        ...
        if (selected_contract_id == 12 || realId == 26) {
          for (let i = 2; i < res.length - 1; i++) res[i] = res[i + 1];
        ...
        var Type = "";
        if (selected_contract_id == 12) Type = "facture";
        if (realId == 25) Type = "devis";
        if (selected_contract_id == 37) Type = "CIVP";
      ...
      try {
        ...
        if (allAnswer[currentQuestionIndex] == "***") {
          if (
            ...
            await axios.post(`${Shared}/api/answers/updateAnswer`, {
              content: ans1 ? text : customInput,
              questions_id: initialQuestionId + (currentQuestionIndex + 1),
            });
...
*/
  ...
  const renderView = () => {
    ...
    if (
      ...
      (language != "Francais" && questionId == 364)
    ) {
      return (
        <View stlye={{}}>
          <View
